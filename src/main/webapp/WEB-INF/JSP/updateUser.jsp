...
$(function(){
	var userId = "${userId}";
	$.ajax({
		url:"${pageContext.request.contextPath}/user/getInfo?userId="+userId,
		type:"post",
		dataType:"json",
		success:function(data){
			...
			$(data.rlist).each(function(){
				if(this.rid==user.idtype){
					$("select#idtype").append("<option value="+this.rid+" selected>"+this.rname+"</option>");
				}else{
...
<span><input type="text" name="userName" class="textbox" required oninvalid="oc(this);" onblur="blu(this);" onclick="oe(this);" onkeyup="this.value=this.value.replace(/[/,， #]/g,'')">
